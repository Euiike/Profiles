...
function historySummary(single) {
    const rexMatch = /\[.*?\]/g;
    const rexExec = /\[(.*),(.*),"(.*)".*\]/;
    let currentPrice, lowest30, lowest90, lowest180, lowest365
    ...
    list.forEach((item, index) => {
        if (item.length > 0) {
            ...
            if (index == 0) {
                ...
                lowest365 = { Name: "一年内最低", Price: `¥${String(price)}`, Date: date, Difference: difference(currentPrice, price), price }
            }
            ...
            if (index < 365 && price < lowest365.price) {
                lowest365.price = price
                lowest365.Price = `¥${String(price)}`
                lowest365.Date = date
                lowest365.Difference = difference(currentPrice, price)
            }
        }
    });
    return [lowest30, lowest90, lowest180, lowest365];
}
